...
### Quota & Context Management
- **Prioritize Gemini Flash** for all routine verification, directory scans, and minor file edits. Only invoke Claude for Λ+80 architectural reasoning or complex debugging.
- **Log Pruning**: When handling large logs (ARK, Game logs, raw output), use `grep_search` first. Never `view_file` more than 100 lines of a log without an explicit reason.
- **Context Distillation**: If a session thread becomes long or repetitive, voluntarily summarize into `.context/last_thread.md` to clear the active context window.
