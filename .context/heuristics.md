...
## 🛠️ Architecture & Infrastructure Heuristics (Session 96)
- **Dead-Man's Switch vs Smoke Alarms:** A local backup script that pings Telegram on failure is a smoke detector wired into the exact same circuit it watches. If the server dies completely, the alarm cannot fire. To secure a durable core, invert the logic: the script must ping a remote Healthchecks.io URL on *success*, meaning absolute silence from the server triggers the alert.
